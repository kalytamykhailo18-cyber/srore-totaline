  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 pb-32">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-brand-800">Mi Carrito ({items.length})</h1>
      ...
      <div className="space-y-3 mb-6">
        {items.map((item) => (
          <div key={item.id} className="bg-white rounded-xl border p-3 sm:p-4">
            <div className="flex gap-3">
              <div className="relative w-20 h-20 sm:w-16 sm:h-16 bg-gray-50 rounded-lg flex-shrink-0">
                {item.image ? (
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-800 text-sm leading-tight line-clamp-2">{item.name}</p>
                <p className="text-xs text-gray-400 mt-0.5">SKU: {item.sku}</p>
                <p className="text-sm sm:text-base font-bold text-brand-800 mt-1">{formatPrice(item.price * item.qty)}</p>
              </div>

              <button
                onClick={() => removeItem(item.id)}
                className="p-1 text-gray-400 hover:text-red-500 self-start shrink-0"
                aria-label="Eliminar"
              >
            ...
            {/* Quantity controls — full width row below on mobile */}
            <div className="mt-3 flex items-center justify-end gap-2">
              <span className="text-xs text-gray-400 mr-1">Cantidad</span>
              <button
                onClick={() => updateQty(item.id, item.qty - 1)}
                className="w-9 h-9 flex items-center justify-center border rounded-lg hover:bg-gray-100 text-lg"
              >
                −
              </button>
              <input
                ...
                className="w-16 h-9 text-center text-sm font-medium border rounded-lg focus:outline-none focus:border-brand-800 focus:ring-1 focus:ring-brand-800"
              />
              <button
                onClick={() => updateQty(item.id, item.qty + 1)}
                className="w-9 h-9 flex items-center justify-center border rounded-lg hover:bg-gray-100 text-lg"
      ...
      {/* Total + Checkout */}
      <div className="bg-white rounded-xl border p-4 sm:p-6 sticky bottom-4 shadow-lg">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <span className="text-base sm:text-lg font-medium text-gray-700">Total</span>
          <span className="text-xl sm:text-2xl font-bold text-brand-800">{formatPrice(totalPrice)}</span>
        </div>
        <button onClick={handleCheckout} className="btn-whatsapp w-full justify-center text-base sm:text-lg py-3">
